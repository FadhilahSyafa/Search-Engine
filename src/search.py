...
class Search():
    ...
    def get_article(self, docs_id):
        ...
        article = {}
        url = re.search(r'(?<=\<url\>).*?(?=\<\/url\>)', file)
        title = re.search(r'(?<=\<title\>).*?(?=\<\/title\>)', file)
        top = re.search(r'(?<=\<top\>).*?(?=\<\/top\>)', file)
        middle = re.search(r'(?<=\<middle\>).*?(?=\<\/middle\>)', file)
        bottom = re.search(r'(?<=\<bottom\>).*?(?=\<\/bottom\>)', file)
